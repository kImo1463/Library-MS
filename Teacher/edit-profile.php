...
if (isset($_POST['submit'])) {
    ...
    $query = "UPDATE teachers SET `full-name`=?, email=?, `phone-number`=? WHERE username=?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ssss", $fullName, $email, $phoneNumber, $_SESSION["username"]);
    $stmt->execute();
    ...
    if ($stmt->affected_rows > 0) {
        ...
        $error = "Failed to update the teacher's profile.";
    }
